...
def _z_from_level(level: float) -> float:
    ...
    """
    x = float(level)
    # Clamp extreme bounds to prevent math domain logarithm errors
    x = max(min(x, 0.999999), 1e-6)

    # Winitzki approximation for erfinv(x)
    a = 0.147
    ln = np.log(1.0 - x * x)
    first = 2.0 / (np.pi * a) + ln / 2.0
    second = ln / a
    erfinv_x = np.sign(x) * np.sqrt(np.sqrt(first * first - second) - first)

    # ✅ CRITICAL MATHEMATICAL FIX: scale by sqrt(2) to yield the actual standard normal z-score
    return float(np.sqrt(2.0) * erfinv_x)
...
def normal_pi(yhat: np.ndarray, sigma: float, level: float = 0.95) -> IntervalResult:
    ...
    # Safety boundary: birth counts cannot drop below zero in rural regions
...
def empirical_pi(yhat: np.ndarray, residuals: np.ndarray, level: float = 0.95) -> IntervalResult:
    ...
    # Safety boundary: birth counts cannot drop below zero
    lower = np.maximum(lower, 0.0)
    return IntervalResult(forecast=yhat, lower=lower, upper=upper, level=level, kind="pi")

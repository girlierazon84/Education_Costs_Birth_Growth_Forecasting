"""src/eduforecast/preprocessing/clean_mortality.py"""
...
def clean_mortality(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise KeyError(f"Mortality missing columns {sorted(missing)}. Found: {list(d.columns)}")
